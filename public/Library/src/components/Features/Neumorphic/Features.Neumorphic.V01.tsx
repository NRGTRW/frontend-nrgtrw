...
export default function FeaturesNeumorphicV01({ 
  title = "Powerful Features",
  subtitle = "Everything you need to succeed",
  description = "Comprehensive tools and capabilities designed to accelerate your growth and maximize efficiency.",
  ...
  const styles = getStyleClasses('neumorphic');
  const vars = { ...defaultColors, ...props.colors };
  
  // Variation knobs for this variant
  const layout = 'split'; // hero, split, grid, showcase
  const density = 'regular'; // compact, regular, roomy
  const ornament = 'moderate'; // minimal, moderate, rich
  const ctaStyle = 'outline'; // solid, outline, ghost
  const typography = 'medium'; // small, medium, large
  const motion = 'fade'; // none, fade, slide
    ...
    if (layout === 'hero') {
      return (
        <div className="relative">
          <InteractiveBadge text="New" variant="neumorphic" />
            ...
            {metrics && metrics.length > 0 && (
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-2xl mx-auto">
                {metrics.map((metric, index) => (
                  <MetricBubble key={index} value={metric.value} label={metric.label} variant="neumorphic" />
                ))}
              </div>
            )}
          </div>
      ...
      return (
        <div className="grid md:grid-cols-2 gap-12 items-center">
          <div className="space-y-6">
            <InteractiveBadge text="Featured" variant="neumorphic" />
      ...
      return (
        <div className="space-y-12">
          <div className="text-center max-w-3xl mx-auto">
            <InteractiveBadge text="Solutions" variant="neumorphic" />
      ...
      return (
        <div className="space-y-12">
          <div className="text-center max-w-4xl mx-auto">
            <InteractiveBadge text="Showcase" variant="neumorphic" />
            <h2 className={`${typographyClasses[typography]} font-light text-gray-900 mb-6`}>
              {title}
            </h2>
            {subtitle && (
