      ...
      <div className="container mx-auto px-4 sm:px-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {items.map((item, index) => (
            <div 
              key={index}
              className="group relative p-6 bg-transparent border border-[var(--primary)]/20 dark:border-[var(--primary-dark)]/20 rounded-lg hover:bg-[var(--card-background-light)] dark:hover:bg-[var(--card-background-dark)] transition-all duration-300"
            >
              <div className="absolute top-4 right-4 w-2 h-2 bg-[var(--primary)] dark:bg-[var(--primary-dark)] rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
