      ...
      <div className="container mx-auto px-4 sm:px-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
          {items.map((item, index) => (
            <div 
              key={index}
              className="group text-center p-6 bg-transparent hover:bg-[var(--card-background-light)] dark:hover:bg-[var(--card-background-dark)] rounded-xl transition-all duration-300"
            >
              <div className="inline-flex items-center justify-center w-16 h-16 bg-[var(--primary)]/10 dark:bg-[var(--primary-dark)]/10 rounded-full mb-4 group-hover:bg-[var(--primary)] dark:group-hover:bg-[var(--primary-dark)] transition-all duration-300">
                {item.icon?.kind === 'icon' ? (
                  <span className="text-2xl group-hover:text-white transition-colors duration-300">{item.icon.src}</span>
