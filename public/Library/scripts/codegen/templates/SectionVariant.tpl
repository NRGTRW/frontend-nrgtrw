...
export default function <%= componentName %>({ 
  ...
  const styles = getStyleClasses('<%= styleSlug %>');
  const vars = { ...defaultColors, ...props.colors };
  
  // Variation knobs for this variant
  const layout = '<%= layout %>'; // hero, split, grid, showcase
  const density = '<%= density %>'; // compact, regular, roomy
  const ornament = '<%= ornament %>'; // minimal, moderate, rich
  const ctaStyle = '<%= ctaStyle %>'; // solid, outline, ghost
  const typography = '<%= typography %>'; // small, medium, large
  const motion = '<%= motion %>'; // none, fade, slide
    ...
    if (layout === 'hero') {
      return (
        <div className="relative">
          <InteractiveBadge text="New" variant="<%= styleSlug %>" />
            ...
            {metrics && metrics.length > 0 && (
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-2xl mx-auto">
                {metrics.map((metric, index) => (
                  <MetricBubble key={index} value={metric.value} label={metric.label} variant="<%= styleSlug %>" />
                ))}
              </div>
            )}
          </div>
      ...
      return (
        <div className="grid md:grid-cols-2 gap-12 items-center">
          <div className="space-y-6">
            <InteractiveBadge text="Featured" variant="<%= styleSlug %>" />
      ...
      return (
        <div className="space-y-12">
          <div className="text-center max-w-3xl mx-auto">
            <InteractiveBadge text="Solutions" variant="<%= styleSlug %>" />
      ...
      return (
        <div className="space-y-12">
          <div className="text-center max-w-4xl mx-auto">
            <InteractiveBadge text="Showcase" variant="<%= styleSlug %>" />
            <h2 className={`${typographyClasses[typography]} font-light text-gray-900 mb-6`}>
              {title}
            </h2>
            {subtitle && (
