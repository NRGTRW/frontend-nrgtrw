...
// Link schema - lenient URL validation
const LinkSchema = z.object({
  ...
  href: z.string().min(1, 'Link href is required').refine(
    (url) => {
      ...
      if (url.startsWith('javascript:') || url.startsWith('data:') || url.startsWith('vbscript:')) return false; // Block dangerous protocols
      try {
        new URL(url);
...
const TestimonialsPropsSchema = z.object({
  ...
  })).min(2, 'At least 2 testimonials are required').max(4, 'Maximum 4 testimonials allowed')
});
...
export function validateAIPlan(plan: unknown): z.infer<typeof AIPlanSchema> {
  return AIPlanSchema.parse(plan);
}

export function validateSectionProps(sectionId: SectionID, props: unknown): unknown {
  const schemas = {
    navbar: NavbarPropsSchema,
    hero: HeroPropsSchema,
    socialProof: SocialProofPropsSchema,
    features: FeaturesPropsSchema,
    featureSpotlight: FeatureSpotlightPropsSchema,
